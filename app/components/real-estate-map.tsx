...
const RealEstateMap: React.FC<RealEstateMapProps> = ({
  ...
  if (cityData && cityData["2022"] && cityData["2023"]) {
    ...
    marketTrend = `${percentIncrease >= 0 ? "+" : ""}${percentIncrease.toFixed(
      1
    )}%`;
  }
  ...
  return (
    <Card className="mt-4 text-2xl">
      <CardHeader>
        <CardTitle>
          {cityName ? `${cityName} Insights` : "Real Estate Insights"}
        </CardTitle>
      </CardHeader>
      <CardContent>
          ...
          <li className="flex items-center space-x-2 text-sm">
            ...
            <div>
              <h3 className="font-semibold">
                Median Real Estate Price for {year}
              </h3>
              <p className="text-2xl text-turquoise">
